...
Notice the presence of column `countrycode` as we grouped by country. If a grid cell falls in multiple countries, it will be repeated for each country. It happens few times:

```{r cells_with_multiple_countries}
cube |>
  dplyr::distinct(eeacellcode, countrycode) |>
  dplyr::add_count(eeacellcode) |>
  dplyr::filter(n > 1) |>
...
```{r remove-ua-ru}
cube <- cube |>
  dplyr::filter(!countrycode %in% c("UA", "RU"))
```
...
Remove rows not associated with an EEA cell code (countries: Azerbaijan, Georgia, Russia):
...
Please go to [`./data/output/GAM_outputs`](https://github.com/damianooldoni/euroraccoon-prototype/tree/main/data/output/GAM_outputs) to download the plots shown in this section.

### Standard plots

In this section we show the plots as returned by `apply_gam()`. Folder: [`./data/output/GAM_outputs`](https://github.com/damianooldoni/euroraccoon-prototype/tree/main/data/output/GAM_outputs).
...
We show and save plots with the species only as title. We save them in sub folder [`./data/output/GAM_outputs/short_title`](https://github.com/damianooldoni/euroraccoon-prototype/tree/main/data/output/GAM_outputs/short_title).
  ...
  purrr::iwalk(x, function(y, sp) {
    ...
    ggplot2::ggsave(
      filename = here::here(
        ...
        "GAM_outputs",
        "short_title",
        paste0(sp, "_", country, "_occurrences_short_title.png")),
      plot = y$plot,
      width = plot_dimensions$width,
      height = plot_dimensions$height,
      units = "px"
  ...
  purrr::iwalk(x, function(y, sp) {
    ...
    ggplot2::ggsave(
      filename = here::here(
        ...
        "GAM_outputs",
        "short_title",
        paste0(sp, "_", country, "_occupancy_short_title.png")),
      plot = y$plot,
      width = plot_dimensions$width,
      height = plot_dimensions$height,
      units = "px"
...
For each country, we can show the plots of the number of occurrences and the measured occupancy next to each other. Plots saved in subfolder [`./data/output/GAM_outputs/plots_for_countries`](https://github.com/damianooldoni/euroraccoon-prototype/tree/main/data/output/GAM_outputs/plots_for_countries).
...
purrr::walk2(
  ...
  function(country, code) {
    purrr::walk(country, function(x) {
      ...
      p <- patchwork::wrap_plots(x$occurrences$plot, x$ncells$plot, nrow = 1, ncol = 2) +
        # Unify legends
        patchwork::plot_layout(guides = 'collect') + 
        # Add general title
        patchwork::plot_annotation(title = code)
      ggplot2::ggsave(
        filename = here::here(
          "data",
          "output",
